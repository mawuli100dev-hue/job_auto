...
5. Sauvegarde les données EXACTES du CV généré (cv_data.json) pour generate_letter.py.

CORRECTIF (bug de matching par sous-chaîne, historique) :
Le matching de mots-clés se fait par MOT ENTIER (regex \\b...\\b).

CORRECTIF v2 (2026-09-14 - SUPPORT DES CANDIDATURES SPONTANÉES) :
Contexte : le pipeline "candidatures spontanées" (build_offres_spontanees.py)
génère des lignes CSV avec le même format que les vraies offres, MAIS avec
un champ "compétences" vide et une "description" réduite à 3-4 phrases
génériques sur l'entreprise (pas d'exigences techniques précises, puisqu'il
n'y a pas de poste publié). Résultat mécanique déjà géré : keyword_bonus()
renvoie ~0 partout, donc llm_rerank_competences() (le fallback déjà existant)
se déclenche systématiquement. Mais les PROMPTS envoyés au LLM pour noter les
expériences et reclasser les compétences étaient écrits en supposant qu'il y
a toujours des exigences techniques précises à faire correspondre - sans ça,
le LLM a très peu de signal et peut faire des choix presque aléatoires.

Ajout : est_candidature_spontanee(offer) détecte ce cas (via type_contrat ou
préfixe d'id "SPONT-") et bascule score_experiences(), llm_rerank_competences()
et generate_sous_titre() sur une consigne adaptée : en l'absence d'exigence
technique explicite, privilégier les expériences/compétences les plus fortes
et les plus polyvalentes du candidat (plutôt que de chercher un mot-clé
technique absent du texte), et rédiger un sous-titre de candidature spontanée
au lieu de reprendre un intitulé de poste qui n'existe pas.
AUCUN changement de style ni de contenu du CV lui-même (cv_builder.py
inchangé) : seule la LOGIQUE DE SÉLECTION en amont est adaptée.

Usage :
python tailor_cv.py --csv "data/offres_filtrees/2026-09-10/....csv" --offer-id 213QBYH
python tailor_cv.py --csv "data/offres_spontanees/....csv" --offer-id SPONT-nom-entreprise
"""
...
DISPONIBILITE_DEFAUT = "dès maintenant"
...
def est_candidature_spontanee(offer: dict) -> bool:
    """Détecte une ligne produite par build_offres_spontanees.py : soit via le
    champ type_contrat ('Candidature spontanée'), soit via le préfixe d'id
    ('SPONT-') utilisé par slugify_id() dans ce même script."""
    type_contrat = normalize(offer.get("type_contrat", ""))
    offer_id = offer.get("id", "") or ""
    return "candidature spontanee" in type_contrat or offer_id.startswith("SPONT-")
...
def generate_sous_titre(client: OpenAI, model: str, offer: dict) -> str:
    """Rédige le sous-titre du CV. En mode candidature spontanée (v2), le
    prompt bascule : il n'y a pas d'intitulé de poste réel à reprendre, donc
    on génère une accroche de candidature spontanée ciblant l'entreprise et
    le domaine visé par le candidat, plutôt que de résumer un intitulé
    générique type 'Candidature spontanée - Alternance Data/IA'."""
    intitule_brut = offer.get("intitule", "") or "Data Analyst"
    entreprise = offer.get("entreprise", "") or "l'entreprise"
    date_dispo = extract_start_date(offer.get("description", "")) or DISPONIBILITE_DEFAUT
    spontanee = est_candidature_spontanee(offer)

    if spontanee:
        prompt = f"""Tu rédiges le SOUS-TITRE d'un CV (une seule phrase courte, affichée juste sous le nom
du candidat), pour une CANDIDATURE SPONTANÉE à une alternance en science des données / intelligence
artificielle chez l'entreprise "{entreprise}". Il n'y a PAS d'offre publiée ni d'intitulé de poste
précis à reprendre : c'est le candidat qui se propose spontanément.

DATE DE DISPONIBILITÉ DU CANDIDAT : {date_dispo}

CONSIGNES :
1. Rédige UNE SEULE phrase, en bon français naturel, qui indique clairement qu'il s'agit d'une
candidature spontanée et mentionne le domaine visé (science des données / intelligence artificielle),
SANS inventer d'intitulé de poste précis qui n'existe pas.
2. Exemple de structure (à ne pas recopier tel quel) : "Candidature spontanée pour une alternance en
science des données, disponible à partir de {date_dispo}".
3. Vise 8 à 14 mots, jamais une phrase à rallonge.
4. INTERDICTION ABSOLUE d'utiliser "—" ou "–". Utilise des virgules ou un tiret simple "-" si besoin.
5. Ne mentionne pas le nom de l'entreprise dans cette phrase (il apparaît déjà ailleurs dans la
candidature) : reste centré sur le domaine visé et la disponibilité.
...
{{"sous_titre": "..."}}"""
    ...
    if spontanee:
        return remove_forbidden_chars(
            f"Candidature spontanée pour une alternance en science des données, disponible à partir de {date_dispo}"
        )
...
def llm_rerank_competences(client: OpenAI, model: str, pool: dict, offer_summary: str, spontanee: bool = False) -> dict:
    """Reclasse les compétences par pertinence quand aucun mot-clé n'a matché
    (systématique en mode candidature spontanée). v2 : consigne dédiée pour
    ce mode, qui privilégie la polyvalence plutôt qu'une pertinence inventée."""
    categories_desc = ""
    for category, config in pool.items():
    ...
    if spontanee:
        consigne_specifique = """CONTEXTE IMPORTANT : il s'agit d'une CANDIDATURE SPONTANÉE. L'offre ci-dessus ne décrit
PAS un poste précis avec des exigences techniques détaillées (juste l'entreprise elle-même) : il n'y a
donc AUCUN mot-clé technique fiable à faire correspondre. Ne tente pas d'inventer une pertinence
artificielle avec le secteur de l'entreprise. Classe plutôt chaque catégorie en mettant en AVANT les
compétences les plus FORTES et les plus POLYVALENTES du candidat (celles qui font la meilleure
vitrine générale de son profil data/IA junior), pour maximiser l'impact quel que soit le poste réel
qui pourrait s'ouvrir chez cette entreprise."""
    else:
        consigne_specifique = "Pour CHAQUE catégorie ci-dessous, classe les compétences par pertinence décroissante pour cette offre (même si le lien n'est pas évident, choisis un ordre plausible) :"
...
def score_experiences(client: OpenAI, model: str, offer_summary: str, experiences: list[dict], spontanee: bool = False) -> list[dict]:
    """Note les expériences pour l'offre. v2 : en mode candidature spontanée,
    le critère n°1 (matcher des outils/méthodes EXPLICITEMENT cités) est
    remplacé par un critère de force/polyvalence générale, puisqu'il n'y a
    pas d'exigence technique précise à matcher."""
    exp_summary = "\n".join(
    ...
    if spontanee:
        criteres = """CRITÈRES DE SÉLECTION (dans cet ordre d'importance) :
1. IMPORTANT : il s'agit d'une CANDIDATURE SPONTANÉE, l'offre ci-dessus ne décrit qu'une entreprise
(pas de poste précis ni d'exigences techniques détaillées). Ne cherche PAS à faire correspondre un
mot-clé technique qui ne figure pas explicitement dans l'offre : privilégie plutôt les expériences
qui démontrent le NIVEAU TECHNIQUE LE PLUS SOLIDE et la plus grande POLYVALENCE du candidat en
science des données/IA, pour constituer la meilleure vitrine générale possible.
2. Calibrage de niveau : le poste est un poste JUNIOR/ALTERNANCE. Évite les expériences en décalage
de niveau, tout en restant convaincant pour décrocher un entretien.
3. Diversité utile : préfère un ensemble complémentaire (des domaines/outils différents) plutôt que
plusieurs expériences démontrant exactement la même compétence, pour montrer l'étendue du profil."""
    else:
        criteres = """CRITÈRES DE SÉLECTION (dans cet ordre d'importance) :
1. Pertinence des compétences et outils : privilégie fortement les expériences qui utilisent des
outils, méthodes ou domaines EXPLICITEMENT cités dans l'offre (y compris des domaines comme l'IA,
le machine learning ou l'automatisation si l'offre les mentionne).
2. Calibrage de niveau : le poste est un poste JUNIOR/ALTERNANCE. Évite les expériences en décalage
de niveau, tout en restant convaincant pour décrocher un entretien.
3. Diversité utile : préfère un ensemble complémentaire plutôt que plusieurs expériences démontrant
exactement la même compétence."""
...
def main():
    ...
    if spontanee:
        print("Mode détecté : CANDIDATURE SPONTANÉE (sélection orientée polyvalence).")
    ...
    raw_selections = score_experiences(client, args.model, offer_summary, experiences, spontanee=spontanee)
    ...
    if total_matches > 0 and not spontanee:
        ...
        if spontanee:
            print(" Mode candidature spontanée : reclassement LLM orienté polyvalence...")
        else:
            print(" Aucune correspondance par mots-clés — appel du LLM en secours...")
        llm_order = llm_rerank_competences(client, args.model, pool, offer_summary, spontanee=spontanee)
    ...
    with open(cv_data_path, "w", encoding="utf-8") as f:
        json.dump({
            ...
            "candidature_spontanee": spontanee,
